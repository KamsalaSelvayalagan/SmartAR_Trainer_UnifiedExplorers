...
# ✅ NEW: start temporary session store (Unity results will fill it)
...
class Workout(QWidget):
    ...
    def start_workout_safely(self):
        ...
        # ✅ NEW: start temporary session store (before first exercise = Jumping Jack)
        if not self.trainee_id:
    ...
    def mark_exercise_completed(self, index):
        ...
        if len(self.completed_indices) == 6 and not self.session_completed:
